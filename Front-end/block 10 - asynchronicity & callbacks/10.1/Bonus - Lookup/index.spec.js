...
const lookup = (login, property) => {
  ...
  const propExists = users.some(user=> user[property]);
  if (!userExists) throw new Error('Could not find user');
  if (!propExists) throw new Error('Could not find property');
  if (propExists && userExists) return users.find(user => user.login === login)[property];
};
