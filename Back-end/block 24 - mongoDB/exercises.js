...
db.movies.updateMany({ title: {$in: ['Home Alone', 'Batman'] }}, {$max: { imdbRating: 17 }});
...
db.xmen.updateMany({ $or: [{occupation: 'Senior Staff', power: { $gt: 100 }}, {occupation: 'Junior Staff', power: { $gt: 200 }}] }, { $set: { area: [ "Students Room" ] }, $currentDate: { lastUpdate: true } });
...
db.xmen.updateMany({occupation: 'Junior Staff', area: { $exists: 0 }}, { $set: { area: [ "outside" ] }, $currentDate: { lastUpdate: true } });
